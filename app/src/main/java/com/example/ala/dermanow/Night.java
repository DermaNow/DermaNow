...
//--------------------------------------------------------------------------------------------------
    ...
    public View onCreateView(LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View root = inflater.inflate(R.layout.night, container, false);
        linearLayout = (LinearLayout) root.findViewById(R.id.linearLayout_night);
        confirm = (Button) root.findViewById(R.id.confirm_button_night);
        confirm.setOnClickListener(this);
        reset = (Button) root.findViewById(R.id.reset_button_night);
    ...
    private boolean store(ViewGroup parent) {
        ...
        for (int i = 0; i < parent.getChildCount(); i++) {
            ...
            if (child instanceof CheckBox) {
                ...
                boolean choice = checkBox.isChecked();
                String index = String.valueOf(i);
                String key = shift + index;
                SharedPrefManager.getInstance(getContext()).saveState(key, choice);
                String time = checkBox.getText().toString().substring(0, 9);
                if (choice)
                    workHours.add(time);
            }
        }
